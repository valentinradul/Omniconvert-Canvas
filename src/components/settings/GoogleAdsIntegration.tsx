...
import { 
  ...
  GoogleAdsCampaign,
  GoogleAdsConfig 
} from '@/hooks/useGoogleAdsIntegration';
...
export const GoogleAdsIntegration: React.FC = () => {
  ...
  useEffect(() => {
    if (status.isConnected && status.config) {
      setCustomerId(status.config.customerId || '');
  ...
  const handleConnect = async () => {
    if (!accessToken.trim()) {
      toast.error('Please enter your OAuth Access Token');
      return;
    }
    if (!customerId.trim()) {
    ...
    try {
      // Test connection
      await testConnection(accessToken, customerId, developerToken);
      
      // Fetch campaigns
      const fetchedCampaigns = await getCampaigns(accessToken, customerId, developerToken);
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      await saveConfig(accessToken, refreshToken, customerId, developerToken, config);
      toast.success('Configuration saved successfully!');
      setWizardStep('complete');
      // Clear sensitive tokens after saving
      setAccessToken('');
      setRefreshToken('');
      setDeveloperToken('');
    } catch (error: any) {
          ...
          <div className="flex items-center gap-2">
            ...
            {status.config?.customerId && (
              <span className="text-sm text-muted-foreground">
                Customer ID: {status.config.customerId}
              </span>
            )}
          </div>
          {canManage && (
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will remove the Google Ads integration and stop syncing campaign data. Your existing data will not be deleted.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      {wizardStep === 'connect' && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Get a developer token from the{' '}
              <a 
          ...
          <div className="space-y-2">
            <Label htmlFor="refreshToken">OAuth Refresh Token (Optional)</Label>
            <Input
              id="refreshToken"
              type="password"
              placeholder="Enter your OAuth 2.0 refresh token"
              value={refreshToken}
          ...
          <Button 
            ...
            disabled={isConnecting || !accessToken.trim() || !customerId.trim() || !developerToken.trim()}
            className="w-full"
          >
            {isConnecting ? (
              <>
      ...
      {wizardStep === 'campaigns' && (
        <div className="space-y-4">
          ...
          <div className="border rounded-lg max-h-64 overflow-y-auto">
            {campaigns.length === 0 ? (
              <p className="p-4 text-sm text-muted-foreground">No campaigns found in this account</p>
            ) : (
              campaigns.map(campaign => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {campaign.status} {campaign.channelType && `• ${campaign.channelType}`}
                    </p>
                  </div>
                </div>
              ))
