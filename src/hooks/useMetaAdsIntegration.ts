...
export function useMetaAdsIntegration() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const configData = data.config as unknown as MetaAdsConfig | null;
        setStatus({
  ...
  const testConnection = async (accessToken: string, adAccountId: string): Promise<boolean> => {
    if (!selectedCompanyId || !session?.access_token) {
      throw new Error('Not authenticated or no company selected');
    }

    const response = await supabase.functions.invoke('fetch-meta-ads', {
      body: {
  ...
  const getCampaigns = async (accessToken: string, adAccountId: string): Promise<MetaAdsCampaign[]> => {
    if (!selectedCompanyId || !session?.access_token) {
      throw new Error('Not authenticated or no company selected');
    }
