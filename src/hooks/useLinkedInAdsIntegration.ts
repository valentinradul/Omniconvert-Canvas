...
export function useLinkedInAdsIntegration() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const configData = data.config as unknown as LinkedInAdsConfig | null;
        setStatus({
  ...
  const testConnection = async (accessToken: string, accountId: string): Promise<boolean> => {
    if (!selectedCompanyId || !session?.access_token) {
      throw new Error('Not authenticated or no company selected');
    }

    const response = await supabase.functions.invoke('fetch-linkedin-ads', {
      body: {
  ...
  const getCampaigns = async (accessToken: string, accountId: string): Promise<LinkedInAdsCampaign[]> => {
    if (!selectedCompanyId || !session?.access_token) {
      throw new Error('Not authenticated or no company selected');
    }
